...
export type ProductRow = {
  ...
  categoryId?: string;    // UNSPSC Kategori ID
  categoryName?: string;  // Kategori adı (gösterim için)
  quantity: number;
  unit: string;
  unitPrice: number;
  extraCosts: number;
...
export default function ItemsSection({
  ...
  // Kategori Arama State'leri
  const [catSearchQueries, setCatSearchQueries] = useState<Record<string, string>>({});
  const [catSearchResults, setCatSearchResults] = useState<Record<string, Category[]>>({});
  ...
  const searchProducts = useCallback(async (rowId: string, query: string) => {
    ...
    try {
      ...
      const localProducts: CatalogProduct[] = (productCatalog || [])
        .filter(p => p.sku?.toLowerCase().includes(query.toLowerCase()) || p.name.toLowerCase().includes(query.toLowerCase()))
        .map((p, i) => ({
      ...
      localProducts.forEach(lp => { if (!combined.some(ap => ap.sku === lp.sku)) combined.push(lp); });

      flushSync(() => { setSearchResults(prev => ({ ...prev, [rowId]: combined })); });
    } catch (e) {
  ...
  // Kategori Arama - Debounced
  const searchCategories = useCallback(async (rowId: string, query: string) => {
    if (!query || query.length < 2) {
      setCatSearchResults(prev => ({ ...prev, [rowId]: [] }));
      return;
    }

    setCatLoading(prev => ({ ...prev, [rowId]: true }));
    try {
      const res = await fetch(`/api/urun/kategori?flat=true&search=${encodeURIComponent(query)}`);
      if (res.ok) {
        const data = await res.json();
        ...
        flushSync(() => { setCatSearchResults(prev => ({ ...prev, [rowId]: filtered })); });
      }
    } catch (e) {
      console.error("Category search error:", e);
    } finally {
      setCatLoading(prev => ({ ...prev, [rowId]: false }));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    Object.entries(catSearchQueries).forEach(([key, query]) => {
      timers[`cat_${key}`] = setTimeout(() => searchCategories(key, query), 300);
    });
    return () => Object.values(timers).forEach(clearTimeout);
  }, [searchQueries, catSearchQueries, searchProducts, searchCategories]);

  const selectProduct = (rowId: string, product: CatalogProduct) => {
    let unitId = unitOptions[0]?.id || "u1";
    if (product.defaultUnit) {
      const matchedUnit = unitOptions.find(u => u.label.toLowerCase() === product.defaultUnit?.toLowerCase());
      if (matchedUnit) unitId = matchedUnit.id;
    }
    updateItem(rowId, { name: product.name, sku: product.sku, productId: product.id, unitPrice: product.estimatedPrice || 0, unit: unitId });
    setSearchQueries(prev => ({ ...prev, [rowId]: "", [`name_${rowId}`]: "" }));
    setSearchResults(prev => ({ ...prev, [rowId]: [], [`name_${rowId}`]: [] }));
  };

  const selectCategory = (rowId: string, category: Category) => {
    updateItem(rowId, { categoryId: category.id, categoryName: category.name });
    setCatSearchQueries(prev => ({ ...prev, [rowId]: "" }));
    setCatSearchResults(prev => ({ ...prev, [rowId]: [] }));
  };
  ...
  return (
    <div className="lg:col-span-2 relative z-0">
          ...
          <table className="min-w-full w-full table-fixed">
            ...
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Kategori (UNSPSC)</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((pr, idx) => (
                <tr key={pr.id} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    >
                      {searchResults[`name_${pr.id}`]?.map((product) => (
                        ...
                        >
                          ...
                          <div className="font-mono text-xs text-blue-600">{product.sku}</div>
                        </button>
                      ))}
                    </DropdownPortal>
                  </td>
                  ...
                  {/* Kategori - Arama ile */}
                  <td className="p-2 align-top" data-dropdown-container>
                    <div ref={(el) => { inputRefs.current[`cat_${pr.id}`] = el as any; }}>
                      <Input
                        size="sm"
                        value={pr.categoryId ? pr.categoryName : (catSearchQueries[pr.id] ?? "")}
                        onChange={(e) => {
                          const value = e.target.value;
                          if (pr.categoryId) {
                            updateItem(pr.id, { categoryId: undefined, categoryName: undefined });
                          }
                          setCatSearchQueries(prev => ({ ...prev, [pr.id]: value }));
                        }}
                        placeholder="Kategori..."
                        className={`bg-transparent ${pr.categoryId ? "text-blue-700 font-bold" : ""}`}
                      />
                    </div>
                    <DropdownPortal
                      anchorRef={{ current: inputRefs.current[`cat_${pr.id}`] }}
                      isOpen={!pr.categoryId && (catSearchResults[pr.id]?.length || 0) > 0}
                    >
                      {catSearchResults[pr.id]?.map((cat) => (
                        <button
                          key={cat.id}
                          type="button"
                          onClick={() => selectCategory(pr.id, cat)}
                          className="w-full text-left px-3 py-2 hover:bg-sky-50 border-b border-slate-100 last:border-0 transition-colors"
                        >
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-slate-700 font-medium">{cat.name}</span>
                            <span className="text-[10px] font-mono bg-sky-100 text-blue-700 px-1 rounded">{cat.code}</span>
                          </div>
                        </button>
                      ))}
                    </DropdownPortal>
                  ...
                  <td className="p-2 align-top">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (num == null && masked !== "") {
                          setErrors((er) => ({ ...er, [pr.id]: { ...(er[pr.id] || {}), quantity: "Gçsz" } }));
                        } else {
                  ...
                  <td className="p-2 align-top">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const num = parseDecimalFlexible(masked);
                        if (num == null && masked !== "") {
                          setErrors((er) => ({ ...er, [pr.id]: { ...(er[pr.id] || {}), unitPrice: "Gçsz" } }));
                        } else {
                  ...
                  {/* Toplam */}
                  <td className="p-2 align-top">
                    <div className="px-3 py-2 text-xs font-bold text-slate-700 text-right bg-slate-100 rounded-lg">
      ...
      <p className="mt-2 text-[10px] text-slate-400 flex items-center gap-1">
        ...
        Ürün ve Kategori alanlarına yazarak sistemden arama yapabilirsiniz.
      </p>
    </div >
  );
}
